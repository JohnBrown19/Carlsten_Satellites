...
def mock_stars_simple_pops(isoch, logage=10.00, met=-2.19174, mass=5e5,  minmass=0.55, 
    ...
    #LogAge to look at one particular age
    #met for metallicity
    #Mass is what we are using to sample and interpolate the Int_IMF by
...
def apply_dist_mod(mock_df, dist_mod):
    """
    Adding distance modulus to the F606 & F814 columns
    """
    new_df = mock_df.copy()
...
#### clean data using culls #######
# def clean_data(data_apt):
#     data_mask = ((data_apt['acs_f606w_snr'] >= 4.0)
#     & (data_apt['acs_f814w_snr'] >= 4.0)
#     & (data_apt['acs_f606w_err'] <= 3.0)
#     & (data_apt['acs_f814w_err'] <= 3.0)
#     & (data_apt['acs_f606w_sharp']**2 < 0.2)
#     & (data_apt['acs_f814w_sharp']**2 < 0.2)
#     & (data_apt['acs_f814w_crowd'] < 0.15 ) 
#     & (data_apt['acs_f606w_crowd'] < 0.15)
#     & (data_apt['objtype_gl'] <= 2)
#     & (data_apt['acs_f606w_vega'] < 99.0) #Data
#     & (data_apt['acs_f814w_vega'] < 99.0) #DAta
#     )
        
#     data_apt_clean = data_apt[data_mask]
#     return data_apt_clean
...
#set 
def compute_ast_completeness_table(ast_apt, bin_width=0.1, mag_min = 22.0, mag_max=29.0, return_interp=False):
    """
    Computes AST completeness for both F606W and F814W bands.

    Parameters
    ----------
    ast_df : astropy.table.Table
    ...
    completeness_table : astropy.table.Table
        Table with magnitude bin centers, completeness, and median error for both filters.
...
def plot_ast_completeness(completeness_tbl):
    ...
    plt.plot(completeness_tbl['mag_bin_center'], completeness_tbl['completeness_814'],
             label='F814W', color='red', linestyle='--')
    ...
    plt.title("AST Completeness Curves")
    plt.ylim(0, 1.05)
    plt.legend()
    plt.grid(True)
    plt.show()
...
def apply_ast_probabilistic_interp_df(
    ...
    # Bias/scatter as function of magnitude
...
def make_and_eval_mock_pop(isoch, fixed_age, met, dmod=29.74, log_mass=6.98, interp_mag_to_comp=None, bias_interp_814=None,
    scatter_interp_814=None, bias_interp_606=None, scatter_interp_606=None, smooth=False, smooth_factor = 1.e1, weight=1.0):

    """
    create and evaluate my mock population for a give age, metallicity
    """
...
# #### color spread plus plots #####

# def color_spread_weighting(fixed_age = 9.9, mets = [-2.19174, -0.5], smooth=False, smooth_factor=1.e1):
#     #fixed_age = 9.9; mets = [-2.19174, -0.5]
#     start_time = time.time()
#     #blue_color = make_and_eval_mock_pop(fixed_age, mets[0], dmod = dmod, log_mass = log_mass, smooth=True, weight = 1.0)
#     #red_color = make_and_eval_mock_pop(fixed_age, mets[1], dmod = dmod, log_mass = log_mass, smooth=True, weight = 1.0)
#     blue_color = make_and_eval_mock_pop(isoch, fixed_age, mets[0], dmod=dmod, log_mass=log_mass, interp_mag_to_comp=interp_mag_to_comp,
#     bias_interp_814=bias_interp_814, scatter_interp_814=scatter_interp_814, bias_interp_606=bias_interp_606,
#     scatter_interp_606=scatter_interp_606, smooth=smooth, smooth_factor = smooth_factor, weight=1.0)

#     red_color = make_and_eval_mock_pop(isoch, fixed_age, mets[1], dmod=dmod, log_mass=log_mass, interp_mag_to_comp=interp_mag_to_comp,
#     bias_interp_814=bias_interp_814, scatter_interp_814=scatter_interp_814, bias_interp_606=bias_interp_606,
#     scatter_interp_606=scatter_interp_606, smooth=smooth, smooth_factor = smooth_factor, weight=1.0)
...
#     # ---------------------------
#     # Choose common x-grid
#     # Using data grid is fine; it makes the reference CDF defined exactly where your data lives.
#     # ---------------------------
#     x_grid = data_stars

#     # Evaluate component ECDFs on the shared grid
#     F_blue = ecdf_on_grid(blue_stars, x_grid)
#     F_red  = ecdf_on_grid(red_stars,  x_grid)

#     # Evaluate data ECDF on the same grid (for plotting / CDF-space diagnostics)
#     F_data_on_grid = ecdf_on_grid(data_stars, x_grid)

#     # ---------------------------
#     # Scan weights using 1-sample KS test:
#     # kstest(sample, cdf_callable)
#     # ---------------------------
...
#         # Make mixture CDF callable: Fmix_callable(x) returns CDF value in [0,1]
...
#     res = pd.DataFrame({"weights": w_list, "KS_D_1samp": D_list, "KS_p_1samp": p_list})
...
#     # Plot p-values vs weight (often useful; log scale)
#     plt.figure(figsize=(7, 4))
...
#     # ---------------------------
#     # Plot 2: CDF overlay at best weight
#     # ---------------------------
...
#     print('execution time = %.3f minutes'%( (time.time() - start_time)/60. ))
...
#### Running the Z score tests for comparing Observed and generated star counts ####

def check_count_consistency(n_obs, n_mock):
    # Step 1: Z-score (Normal Approximation)
    # Standard deviation of Poisson is sqrt(mean)
    sigma = np.sqrt(n_mock)
    z_score = (n_obs - n_mock) / sigma
    
    # Step 2: Poisson P-value (Exact)
    # We calculate the probability of getting exactly n_obs stars 
    # given an expectation of n_mock
    p_val_poisson = stats.poisson.pmf(n_obs, n_mock)
    
    # Cumulative probability (Tail test): 
    # Probability of seeing n_obs or more/less by chance
    p_val_extreme = 1 - stats.poisson.cdf(n_obs - 1, n_mock) if n_obs > n_mock else stats.poisson.cdf(n_obs, n_mock)

    print(f"--- Total Count Results ---")
    print(f"Observed: {n_obs}, Mock Prediction: {n_mock}")
    print(f"Z-score: {z_score:.2f}σ")
    print(f"Poisson PMF (Exact Prob): {p_val_poisson:.4f}")
    print(f"Tail Probability (p-value): {p_val_extreme:.4f}")

# Example Usage:
# check_count_consistency(len(data_color), len(mock_pop_color))
###### Running KDE  #########
